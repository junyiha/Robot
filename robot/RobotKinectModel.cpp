...
Eigen::VectorXd CRobotKinectModel::getJointVel(Eigen::VectorXd Tar_Speed, bool Ref_End)
{
    ...
    for (int i = 0; i < m_SerialLink.size(); i++)
    {
        if (m_SerialLink[i].Is_Free() == false)
            continue;
...
Eigen::VectorXd CRobotKinectModel::end2base(Eigen::VectorXd end)
{
    ...
    Eigen::Quaterniond quat = Eigen::Quaterniond(Eigen::AngleAxisd(euler_angles(0), Eigen::Vector3d::UnitX())) * Eigen::Quaterniond(Eigen::AngleAxisd(euler_angles(1), Eigen::Vector3d::UnitY())) * Eigen::Quaterniond(Eigen::AngleAxisd(euler_angles(2), Eigen::Vector3d::UnitZ()));
...
Eigen::VectorXd CRobotKinectModel::getTarSpeed(Eigen::VectorXd tarEndPos)
{
    ...
    rotation_matrix = Eigen::AngleAxisd(tarEndPos[3], Eigen::Vector3d::UnitX()) * Eigen::AngleAxisd(tarEndPos[4], Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(tarEndPos[5], Eigen::Vector3d::UnitZ());
